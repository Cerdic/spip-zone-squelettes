...
global $balise_FORMULAIRE_REPLY_collecte;
$balise_FORMULAIRE_REPLY_collecte = array('id_auteur', 'id_article', 'email');

function balise_FORMULAIRE_REPLY_stat($args, $filtres) {

	// Pas d'id_auteur ni d'id_article ? Erreur de squelette
	if (!$args[0] AND !$args[1])
		return erreur_squelette(
			_T('zbug_champ_hors_motif',
				array ('champ' => '#FORMULAIRE_REPLY',
...
function balise_FORMULAIRE_REPLY_dyn($id_auteur, $id_article, $mail) {
	...
	if ($validable
	AND $id == _request('id_formulaire_reply')) { 
	...
	$link->delVar('id_formulaire_reply');

	return 
		array('formulaire_reply', 0,
			array(
			'action' => $link->getUrl(),
	// *************
	// line below added 
