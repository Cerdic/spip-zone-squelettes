...
unset($_SESSION['devis'][$id]);
		...
		//_________________________________________________________________________
				if(($_POST['id_dernier']!='')&&($_POST['prix-karim']!='')){
				$dernier = $_POST['id_dernier'];
				$_SESSION['devis'][$dernier][quantite] = $_POST['qte_dernier'];
				$_SESSION['devis'][$dernier][prix] = $_POST['prix-karim'];
				}
		//_________________________________________________________________________
		//
		...
		//_________________________________________________________________________
		if($_POST['update']!=''){
			$keys = array_keys($_SESSION['devis']);
			for($i=0; $i <= count($keys)-1; $i ++) {
					$key = $keys[$i];
					$_SESSION['devis'][$key][quantite] = $_POST[$key];
			}
		}
		//_________________________________________________________________________
		//
