...
define('URLS_herbier_EXEMPLE', '/rubrique/titre');
 ...
 * 
 * Choisissez "herbier" dans les pages de configuration d'URL
 *
 ...
 * define ('_terminaison_urls_herbier', '.html');
 * 
 * pour preciser des terminaisons particulieres pour certains types
 * $GLOBALS['url_herbier_terminaisons']=array(
 * 'rubrique' => '/',
 * 'mot' => '',
 * 'groupe' => '/',
 * 'defaut' => '.html');
 ...
 * $GLOBALS['url_herbier_types']=array(
...
define ('_debut_urls_herbier', '');
define ('_terminaison_urls_herbier', '');
define ('_url_herbier_sep_id','-');
define ('_url_herbier_minuscules',1);
...
function url_herbier_parent($type){
	static $parents = null;
	if (is_null($parents)){
		$parents = array(
			  'article'=>array('id_rubrique','rubrique'),
			  ...
			  'breve'=>array('id_rubrique','rubrique'),
			  'site'=>array('id_rubrique','rubrique'));
		if (isset($GLOBALS['url_herbier_parents']) AND !isset($_REQUEST['url_herbier_parents'])){
			$parents = array_merge($parents,$GLOBALS['url_herbier_parents']);
		}			  
	}
	return (isset($parents[$type])?$parents[$type]:'');
}

function url_herbier_terminaison($type){
	static $terminaison_types = null;
	if ($terminaison_types==null){
		$terminaison_types = array('rubrique' => '/','mot' => '','defaut' => defined('_terminaison_urls_herbier')?_terminaison_urls_herbier:'.html');
		if (isset($GLOBALS['url_herbier_terminaisons']))
			$terminaison_types = array_merge($terminaison_types,$GLOBALS['url_herbier_terminaisons']);
	}
...
function url_herbier_type($type){
	// par defaut les rubriques ne sont pas typees, mais le reste oui
	static $synonymes_types = null;
	if (!$synonymes_types){
		$synonymes_types = array('rubrique'=>'');
		if (isset($GLOBALS['url_herbier_types']) AND is_array($GLOBALS['url_herbier_types']))
			$synonymes_types = array_merge($synonymes_types,$GLOBALS['url_herbier_types']);
	}
...
function creer_chaine_url($x) {
	...
	if (strlen($url) < _URLS_herbier_MIN) {
		$url = $objet['id_objet'];
	}
	...
	// Sinon couper les mots et les relier par des tirets
		...
		foreach ($mots as $mot) {
			...
			// Si on depasse _URLS_herbier_MAX caracteres, s'arreter
			// ne pas compter 3 caracteres pour %E9 mais un seul
			$long = preg_replace(',%.,', '', $url2);
			if (strlen($long) > _URLS_herbier_MAX) {
				break;
			}
	...
	$x['data'] = 
		url_herbier_type($objet['type']) // le type ou son synonyme
	  . $url; // le titre
...
// http://doc.spip.org/@declarer_url_herbier_rec
function declarer_url_herbier_rec($url,$type,$parent,$type_parent){
	if (is_null($parent)){
		return $url;
	}
	if($parent==0)
		...
		$url_parent = declarer_url_herbier($type_parent?$type_parent:'rubrique',$parent);
		return rtrim($url_parent,'/') . '/' . rtrim($url,'/');
	}
}

// http://doc.spip.org/@declarer_url_herbier
function declarer_url_herbier($type, $id_objet) {
	static $urls=array();
	...
	if (!isset($urls[$type][$id_objet]) OR $modifier_url) {
		...
		// parent
		$champ_parent = url_herbier_parent($type);
	...
	if (!is_null($url_propre) AND !$modifier_url)
		return declarer_url_herbier_rec($url_propre,$type,$urls[$type][$id_objet]['parent'],$urls[$type][$id_objet]['type_parent']);
	...
	AND $r[1] != $type)
		$url = $url._url_herbier_sep_id.$id_objet;
	...
	if (@sql_insertq('spip_urls', $set) <= 0) {
		...
		do {
			...
			if (sql_countsel('spip_urls', $where  .sql_quote($set['url']))) {
				...
				return declarer_url_herbier_rec($urls[$type][$id_objet]['url']=$set['url'],$type,$urls[$type][$id_objet]['parent'],$urls[$type][$id_objet]['type_parent']);
			}
			else {
				$set['url'] .= _url_herbier_sep_id.$id_objet;
				if (strlen($set['url']) > 200)
					//serveur out ? retourner au mieux
					return  declarer_url_herbier_rec($urls[$type][$id_objet]['url']=$url_propre,$type,$urls[$type][$id_objet]['parent'],$urls[$type][$id_objet]['type_parent']);
	...
	return declarer_url_herbier_rec($urls[$type][$id_objet]['url'],$type,$urls[$type][$id_objet]['parent'],$urls[$type][$id_objet]['type_parent']);
}

// http://doc.spip.org/@_generer_url_herbier
function _generer_url_herbier($type, $id, $args='', $ancre='') {
	...
	// Mode propre
	$propre = declarer_url_herbier($type, $id);

	if ($propre === false) return ''; // objet inconnu. raccourci ? 

	if ($propre) {
		$url = _debut_urls_herbier
			. rtrim($propre,'/')
			. url_herbier_terminaison($type);
	} else {
...
// http://doc.spip.org/@urls_herbier_dist
function urls_herbier_dist($i, $entite, $args='', $ancre='') {
	if (is_numeric($i))
		return _generer_url_herbier($entite, $i, $args, $ancre);
	...
	$t = array_diff(array_unique(array_merge(array('.html','.htm','/'),url_herbier_terminaison(''))),array(''));
	if (count($t))
	...
	if (strlen($url_propre) AND !preg_match(',^[^/]*[.]php,',$url_propre)){
		...
		// recuperer tous les objets de lherbier xxx/article/yyy/mot/zzzz
		$url_herbier = explode('/',$url_propre);
		while (count($url_herbier)>0){
			$url_propre = array_pop($url_herbier);
			if (count($url_herbier))
				$type = array_pop($url_herbier);
			else
				$type=null;
