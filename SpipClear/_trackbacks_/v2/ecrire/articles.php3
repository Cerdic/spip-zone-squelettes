...
$date_ok = ($post_dates=='oui' OR ($post_dates=='non' AND $date<=date("Y-m-d H:i:s")));
if ($connect_statut=='0minirezo' AND acces_rubrique($id_rubrique) AND $statut_article=='publie' and $date_ok) {
	icone_horizontale(_T('trackbacks:icone_envoi'),
		"envoi_trackback.php?id_article=$id_article", "trackback-24.png", "");
	icone_horizontale(_T('trackbacks:icone_suivi_cet_article'),
		"controle_forum.php3?mode=trackbacks&amp;id_article=$id_article", "trackback-24.png", "");
...
if ($options == "avancees" && $connect_statut=='0minirezo' && $flag_editable) {
	...
	$visible = $change_accepter_forum || $change_petition
		|| $change_accepter_trackback || $nb_forums || $nb_signatures;
	...
	// Reglage existant
	$trackbacks = get_trackbacks($id_article);

	// Modification du reglage ?
	if (isset($change_accepter_trackback)
	AND $change_accepter_trackback <> $trackbacks) {
		$trackbacks = $change_accepter_trackback;
		modifier_trackbacks($id_article, $trackbacks);
	}

	// Afficher le formulaire de modification du reglage
	echo formulaire_modification_trackbacks($id_article, $trackbacks);
