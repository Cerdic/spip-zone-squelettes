...
function formulaires_melusine_chemin_charger(){
	$valeurs = array('d1'=>'menu','d2'=>'boutonsgauche','d3'=>'miseajour','d4'=>'mentionslegales','d5'=>'diaporama');
	melusine_liste_noisettes_dispo("chemin");
	return $valeurs;
}

function formulaires_melusine_chemin_verifier(){
	$erreurs = array();
...
function formulaires_melusine_chemin_traiter(){
	$position=_request('position');
	if($position=='skels'){melusine_liste_noisettes_dispo("chemin");return false;}
	$action=substr($position,0,1);
	$var=substr($position,1);
	
	if($action=="d" && $var<11 ){
		$varplus=$var+1;
		$chemin='melusine_chemin/effectifs/'.$var;
		$chemin_bas='melusine_chemin/effectifs/'.$varplus;
	...
	if($action=="m" && $var>1 ){
		$varmoins=$var-1;
		$chemin='melusine_chemin/effectifs/'.$var;
		$chemin_haut='melusine_chemin/effectifs/'.$varmoins;
		$pos=lire_config($chemin);
	...
	if($action=="s"){
		...
		melusine_rassembler($var,"chemin");
	}
	if($action=="a"){
		$j=1;
		while($valeur!='aucun'){
		$chemin='melusine_chemin/effectifs/'.$j;
	...
	if($action=="i"){
		...
		for ($i=1;$i<3;$i++){
			$j=$i-1;
			$chemin='melusine_chemin/effectifs/'.$i;					
