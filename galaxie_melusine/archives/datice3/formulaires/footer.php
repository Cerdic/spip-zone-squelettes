<?php 
function footer_pasvide($colonne){
...
function footer_skels(){
	effacer_config("datice_footer/skel");
	$squel=array();
	$chemin=_DIR_PLUGIN_DATICE;
	$chemin=$chemin."modules/footer";
	$i=1;
	if ($handle = opendir($chemin)) {    		
    		while (false !== ($file = readdir($handle))) {
		$sk=substr($file,0,-5);
		if($sk!='' && strlen($sk)>3){
			$cheminskels="datice_footer/skel/skels".$i;
...
function footer_rassembler($i){
	$var=$i;
	$chemin='datice_footer/effectifs/'.$var;
	$j=$i+1;
	$varplus=$j;
	$chemin_bas='datice_footer/effectifs/'.$varplus;
	$pos_bas=lire_config($chemin_bas);
	ecrire_config($chemin,$pos_bas);
	ecrire_config($chemin_bas,'aucun');
	$i++;
	if($i<12){footer_rassembler($i);};
}


function formulaires_footer_traiter(){
	$position=_request('position');
	if($position=='skels'){footer_skels();return false;}
	...
	if($action=="m" && $var>1 ){
		$varmoins=$var-1;
		$chemin='datice_footer/effectifs/'.$var;
		$chemin_haut='datice_footer/effectifs/'.$varmoins;
		$pos=lire_config($chemin);
	...
	if($action=="s"){
		...
		footer_rassembler($var,$zone);
	}
	if($action=="a"){
		$j=1;
		while($valeur!='aucun'){
		$chemin='datice_footer/effectifs/'.$j;
	...
	if($action=="i"){
		...
		for ($i=1;$i<3;$i++){
			$j=$i-1;
			$chemin='datice_footer/effectifs/'.$i;					
