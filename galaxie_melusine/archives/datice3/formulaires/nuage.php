...
function formulaires_nuage_charger(){
	$valeurs = array();
	return $valeurs;
}

function formulaires_nuage_verifier(){
	$erreurs = array();
...
function formulaires_nuage_traiter(){
	effacer_config(datice_nuage);
	$couleur=_request('couleur');	
	$chemin_couleur="datice_nuage/couleur";
	...
	if($mots){
		foreach ($mots as $value){
			...
			$chemin_id="datice_nuage/mot/".$tab_mot[0]."/id";
			$chemin_tit="datice_nuage/mot/".$tab_mot[0]."/titre";
			$chemin_taille="datice_nuage/mot/".$tab_mot[0]."/taille";
			if ($titre){
	...
	if($articles){
		foreach ($articles as $value){
			...
			$pattern="/^( [0-9])\. /";
			$titre=preg_replace($pattern,"",$titre);
			$chemin_id="datice_nuage/article/".$tab_article[0]."/id";
			$chemin_tit="datice_nuage/article/".$tab_article[0]."/titre";
			$chemin_taille="datice_nuage/article/".$tab_article[0]."/taille";
			if ($titre){
