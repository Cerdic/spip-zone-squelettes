...
*/
	...
	function Link($tag, $method = "", $text = "", $track = 1) {
		...
		{
			$tag = $this->GetInterWikiUrl($matches[1], $matches[3]);
			return "<a href=\"$tag\">$matches[1]:$matches[3] (interwiki)</a>";
		}
	...
	function ReadInterWikiConfig() {
		if ($lines = file(SPIKINI."interwiki.conf"))
		{
	...
	function Format($text, $formatter = "wakka") {
		if ($formatter == "wakka" OR $formatter == "spip") {
			include_spip("texte.php");
			include_spip("filtres.php");
			include_once('formatters/wakka.php');
			$text = propre($text);

			// liens locaux
			$text = preg_replace(",(<a href\=\")([^>]*. class\=.spip_in.),", "\\1../\\2",$text);
			$text = preg_replace(",(<(a href|img src)\=\')(IMG/),", "\\1../\\3", $text);
		} else
			$text = $this->IncludeBuffered("formatters/".$formatter.".php", "<i>Impossible de trouver le formateur \"$formatter\"</i>", compact("text")); 
	...
	function LoadUser($name, $password = 0) {
		//if ($GLOBALS['auteur_session']['id_auteur'] > 0) return true;
		return $this->LoadSingle("select * from ".$this->config["table_prefix"]."users where name = '".mysql_escape_string($name)."' ".($password === 0 ? "" : "and password = '".mysql_escape_string($password)."'")." limit 1");
	}
	function LoadUsers() { return $this->LoadAll("select * from ".$this->config["table_prefix"]."users order by name"); }
	function GetUserName() {
		if ($user = $this->GetUser()) $name = $user["nom"];
...
if (!$configfile = GetEnv("WAKKA_CONFIG"))
	$configfile = SPIKINI."wakka.config.php";
if (file_exists($configfile)
OR file_exists($configfile = "ecrire/wakka.config.php") # versions plus anciennes de spikini
) {
	include($configfile);
}
...
{
	if ($multi AND !($auteur_session['statut'] == '0minirezo')) {
		die ("<a href='/".SPIP_LOGIN."?var_url=/'>Il faut être administrateur de ce SPIP pour pouvoir créer un wiki.</a>");
	}

	// start installer
	if (!isset($_REQUEST["installAction"]) OR !$installAction = trim($_REQUEST["installAction"])) $installAction = "default";
	include(SPIKINI."setup/header.php");
	if (file_exists(SPIKINI."setup/".$installAction.".php")) include(SPIKINI."setup/".$installAction.".php"); else echo "<i>Invalid action</i>" ;
	include(SPIKINI."setup/footer.php");
	exit;
}
...
// acls addcomment claim deletepage diff edit raw referrers referrers_sites revisions show xml
