...
function formulaire_inscription($type) {
  ...
  global $mail_inscription, $nom_inscription;
	...
	if ($mail_inscription && $nom_inscription) {
		...
		$res = "<p class=\"reponse_formulaire\">";
		...
		// l'abonne existe deja.
	 	if ($row = spip_fetch_array($result)) {
			...
			unset ($continue);
			if ($statut == '5poubelle')
				$res .= "<strong>"._T('form_forum_access_refuse')."</strong>";
			else if ($statut == 'nouveau') {
				spip_query ("DELETE FROM spip_auteurs WHERE id_auteur=$id_auteur");
				$continue = true;
			} else
				$res .= "<strong>"._T('form_forum_email_deja_enregistre')."</strong>";
		} else
			$continue = true;
...
"VALUES ('".addslashes($nom_inscription)."', '".addslashes($mail_inscription)."', '$login', '$mdpass', '$statut', '$htpass')");
		...
		$url = $link->getUrl();
		$url = quote_amp($url);
		$res = "<p>"._T('form_forum_indiquer_nom_email') ."</p>".	
		  ...
		  "<input type=\"text\" name=\"mail_inscription\" value=\"\" id=\"mail_inscription\" /><br />\n" .
...
function formulaire_ecrire_auteur($id_auteur, $email_auteur) {
	...
	if ($GLOBALS['texte_message_auteur'.$id_auteur]) {
		if ($GLOBALS['sujet_message_auteur'.$id_auteur] == "")
			...
			$res = "<fieldset class=\"spip_encadrer\">"._T('form_prop_sujet')." <strong>".entites_html($GLOBALS['sujet_message_auteur'.$id_auteur])."</strong></fieldset>";
			if ($flag_wordwrap)
			...
			$res .= $link->getForm('post') .
			  "<div style=\"text-align:center;\"><input type=\"submit\" name=\"Confirmer\" class=\"spip_bouton\" value=\""._T('form_prop_confirmer_envoi')."\" />" .
			  "</div></form>";
		}
	}

	if ($erreur)
		$res = "<fieldset class='spip_encadrer'><strong class=\"erreur\">$erreur<br />&nbsp;</strong></fieldset>\n";

	if ($affiche_formulaire) {
		$retour = $GLOBALS['REQUEST_URI'];
		$link = $GLOBALS['clean_link'];
		$res .= $link->getForm('post') .
		  "<fieldset class=\"spip_encadrer\"><label for=\"email_message_auteur$id_auteur\" title=\"votre adresse email du type toto@toto.com\">"._T('form_pet_votre_email')."</label><br />" .
		  "<input type=\"text\" class=\"forml\" name=\"email_message_auteur$id_auteur\" id=\"email_message_auteur$id_auteur\" value=\"".entites_html($GLOBALS['email_message_auteur'.$id_auteur])."\"  />\n" .
		  "<label for=\"sujet_message_auteur$id_auteur\" title=\"le sujet de votre message\">"._T('form_prop_sujet')."</label><br />" .
		  "<input type=\"text\" class=\"forml\" name=\"sujet_message_auteur$id_auteur\" id=\"sujet_message_auteur$id_auteur\" value=\"".entites_html($GLOBALS['sujet_message_auteur'.$id_auteur])."\"  />\n" .
		  "<label for=\"texte_message_auteur$id_auteur\" title=\"le texte de votre message\">Votre message</label><br />" .
		  "<textarea name=\"texte_message_auteur$id_auteur\" id=\"texte_message_auteur$id_auteur\" rows=\"10\" class=\"forml\" cols=\"40\" >".entites_html($GLOBALS['texte_message_auteur'.$id_auteur])."</textarea>\n" .
		  "<div style=\"text-align:center;\"><input type=\"submit\" name=\"Valider\" class=\"spip_bouton\" value=\""._T('form_prop_envoyer')."\" /></div>" .
		  "</fieldset></form>";
	}
	lang_dselect();
	return $res;
