...
function formulaires_spipr_educ_responsive_petit_charger_dist() {
	// Layout précédemment choisi
	$test_layout=sql_select('parametre1','spip_spipr_educ',"nom='layout_petit' AND nom_sauvegarde='en_cours_d_utilisation_SPIPr'");
	$tab_test_layout=sql_fetch($test_layout);
	$test_layout_petit=$tab_test_layout['parametre1'];
	if (!$test_layout_petit) {$test_layout_petit=33;}
	// Répertoire des vignettes des LayoutGala
	$chemin =_DIR_PLUGIN_SPIPR_EDUC.'prive/themes/spip/images/LayoutGala/';
	$valeurs=array();
	$valeurs['layout_actuel'] = $test_layout_petit;
	$valeurs['chemin_vignettes'] = $chemin;
	return $valeurs;
}

function formulaires_spipr_educ_responsive_petit_traiter_dist() {
	include_spip('inc/spipr_educ_structure_responsive');
	$largeurs_colonnes = spipr_educ_definition_largeurs_colonnes();
	$res = array('editable'=>true);
	foreach(array("layoutgala_petit") as $layout) {
		sql_updateq(
			'spip_spipr_educ',
			array(
			...
			"nom='layout_petit' AND nom_sauvegarde='en_cours_d_utilisation_SPIPr'"
		);
	}
