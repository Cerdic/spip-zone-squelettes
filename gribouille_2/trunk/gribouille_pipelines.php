		...
		if(is_array($id_secteurs)){
			if ($flux['args']['type']=='article'
			  ...
			  $serveur =  $flux['args']['serveur'];
				$p2 = array();
				$s = sql_select("id_article", "spip_articles", "statut='publie' AND ".sql_in('id_article', array_keys($points), '','','','',$serveur)." AND ".sql_in('id_secteur', array_values($id_secteurs), 'NOT','','','',$serveur));
			...
			if ($flux['args']['type']=='rubrique'
			  ...
			  $serveur =  $flux['args']['serveur'];
				$p2 = array();
				$s = sql_select("id_rubrique", "spip_rubriques", "statut='publie' AND ".sql_in('id_rubrique', array_keys($points), '','','','',$serveur)." AND ".sql_in('id_secteur', array_values($id_secteurs), 'NOT','','','',$serveur));
				while ($t = sql_fetch($s, $serveur)){
					$p2[$t['id_article']] ='';
				}
				$p2 = array_intersect_key($points,$p2);
