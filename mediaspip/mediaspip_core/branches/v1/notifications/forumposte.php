...
function notifications_forumposte_dist($quoi, $id_forum, $options) {
	...
	AND $prevenir_auteurs) {
		...
		while ($qui = sql_fetch($result)) {
			if ($qui['email'] AND autoriser('modererforum', 'article', $t['id_objet'], $qui['id_auteur']))
				$tous[] = $qui['email'];
		}
	}
	...
	// Envoyer les emails
	//
	include_spip('inc/forum_notifications');
	foreach ($destinataires as $email) {
		$texte = email_notification_forums_bis($t, $email);
