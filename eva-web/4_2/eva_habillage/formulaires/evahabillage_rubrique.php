...
function formulaires_evahabillage_rubrique_charger_dist(){
...
function formulaires_evahabillage_rubrique_traiter_dist(){
	...
	if (_request('bloc_rubrique_valider')) {
		$verif_post_bloc=sql_select('*','spip_eva_habillage_images',"type='bloc' AND nom_habillage='Defaut' AND attach='rubrique'");
	...
	if (_request('skel_perso_rubrique')) {
		sql_insertq('spip_eva_habillage_images',array(
			'type' => 'bloc',
			'nom_habillage' => 'Defaut',
			'nom_div' => _request('eva_mon_bloc_perso_nom_rubrique'),
			'nom_image' => _request('eva_mon_bloc_perso_skel_rubrique'),
			'pos_x' => _request('eva_mon_bloc_perso_pos_x_rubrique'),
			'repetition' => 'perso',
			'attach' => 'rubrique'
		));
		$res['message_ok'] = 'Le squelette personnel <b>'._request('eva_mon_bloc_perso_nom_rubrique').'</b> a &eacute;t&eacute; ins&eacute;r&eacute;';
	}
	//On traite enfin la suppression des noisettes perso préalablement insérées
	if (_request('submit_supprime_skel_perso_rubrique')) {
		sql_delete('spip_eva_habillage_images',"id='"._request('eva_suppr_skel_perso_rubrique')."'");
