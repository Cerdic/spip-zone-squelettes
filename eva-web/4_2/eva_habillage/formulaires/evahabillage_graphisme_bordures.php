...
function formulaires_evahabillage_graphisme_bordures_charger_dist(){
...
function formulaires_evahabillage_graphisme_bordures_traiter_dist(){
	...
	if (_request('eva_choix_bordures')) {
		foreach($mon_theme as $habillage_cles => $habillage_inutile) {
			if (strpos($habillage_cles,'bordure_')!==FALSE) {
					sql_updateq('spip_eva_habillage_themes',array($habillage_cles=>_request($habillage_cles)),"nom='Defaut'");
			}
		}
		$res['message_ok'] = 'Modification des caract&eacute;ristiques des bordures enregistr&eacute;e';
	}
	return $res;
}