...
function formulaires_evahabillage_sommaire_charger_dist(){
...
function formulaires_evahabillage_sommaire_traiter_dist(){
	...
	if (_request('bloc_sommaire_valider')) {
		$verif_post_bloc=sql_select('*','spip_eva_habillage_images',"type='bloc' AND nom_habillage='Defaut' AND attach='sommaire'");
	...
	if (_request('skel_perso_sommaire')) {
		sql_insertq('spip_eva_habillage_images',array(
			'type' => 'bloc',
			'nom_habillage' => 'Defaut',
			'nom_div' => _request('eva_mon_bloc_perso_nom_sommaire'),
			'nom_image' => _request('eva_mon_bloc_perso_skel_sommaire'),
			'pos_x' => _request('eva_mon_bloc_perso_pos_x_sommaire'),
			'repetition' => 'perso',
			'attach' => 'sommaire'
		));
		$res['message_ok'] = 'Le squelette personnel <b>'._request('eva_mon_bloc_perso_nom_sommaire').'</b> a &eacute;t&eacute; ins&eacute;r&eacute;';
	}
	//On traite enfin la suppression des noisettes perso préalablement insérées
	if (_request('submit_supprime_skel_perso_sommaire')) {
		sql_delete('spip_eva_habillage_images',"id='"._request('eva_suppr_skel_perso_sommaire')."'");
