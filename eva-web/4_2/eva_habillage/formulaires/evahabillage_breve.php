...
function formulaires_evahabillage_breve_charger_dist(){
...
function formulaires_evahabillage_breve_traiter_dist(){
	...
	if (_request('bloc_breve_valider')) {
		$verif_post_bloc=sql_select('*','spip_eva_habillage_images',"type='bloc' AND nom_habillage='Defaut' AND attach='breve'");
	...
	if (_request('skel_perso_breve')) {
		sql_insertq('spip_eva_habillage_images',array(
			'type' => 'bloc',
			'nom_habillage' => 'Defaut',
			'nom_div' => _request('eva_mon_bloc_perso_nom_breve'),
			'nom_image' => _request('eva_mon_bloc_perso_skel_breve'),
			'pos_x' => _request('eva_mon_bloc_perso_pos_x_breve'),
			'repetition' => 'perso',
			'attach' => 'breve'
		));
		$res['message_ok'] = 'Le squelette personnel <b>'._request('eva_mon_bloc_perso_nom_breve').'</b> a &eacute;t&eacute; ins&eacute;r&eacute;';
	}
	//On traite enfin la suppression des noisettes perso préalablement insérées
	if (_request('submit_supprime_skel_perso_breve')) {
		sql_delete('spip_eva_habillage_images',"id='"._request('eva_suppr_skel_perso_breve')."'");
