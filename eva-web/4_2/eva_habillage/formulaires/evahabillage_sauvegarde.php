...
function formulaires_evahabillage_sauvegarde_traiter_dist() {
	...
	if ((_request('sauvegarder')=='Sauvegarder') AND (_request('sauvegarde_habillage')!='Defaut')) {
		...
		if (strpos($nom_sauvegarde,"'") OR strpos($nom_sauvegarde,'"')) {
			$res['message_erreur'] = 'Le nom de sauvegarde ne doit pas comporter d\'apostrophe ou de guillemet. La sauvegarde n\'a pas &eacute;t&eacute; enregistr&eacute;e';
			$res['message_ok'] = '';
		}
		else {
			...
			sql_delete('spip_eva_habillage','sauvegarde='.sql_quote($nom_sauvegarde));
			sql_insertq('spip_eva_habillage',array('habillage' => $tab_habillage_defaut['habillage'],'sauvegarde' => $nom_sauvegarde));
			sql_delete('spip_eva_habillage_themes','nom='.sql_quote($nom_sauvegarde));
			sql_insertq('spip_eva_habillage_themes',array('nom' => $nom_sauvegarde));
			...
			foreach($def_themes as $habillage_cles => $habillage_inutile) {sql_updateq('spip_eva_habillage_themes',array($habillage_cles => $tab_valeurs[$habillage_cles]),'nom='.sql_quote($nom_sauvegarde));
			}
			sql_delete('spip_eva_habillage_images','nom_habillage='.sql_quote($nom_sauvegarde));
			...
			while ($tab=sql_fetch($result_images_sauve)) {
				sql_insertq("spip_eva_habillage_images",array('type' => $tab['type'],'nom_habillage' => $nom_sauvegarde,'nom_div' => $tab['nom_div'], 'nom_image' => $tab['nom_image'], 'pos_x' => $tab['pos_x'],'pos_y' => $tab['pos_y'],'repetition' => $tab['repetition'], 'attach' => $tab['attach']));
			}
			$res['message_ok'] = 'L\'habillage <b>'.$nom_sauvegarde.'</b> a &eacute;t&eacute; sauvegard&eacute;';
		}
	}
	elseif ((_request('sauvegarder')=='Restaurer') AND (_request('restaurer_habillage')!='Defaut')) {
		$nom_sauvegarde=_request('restaurer_habillage');
		$recherche_habillage_restaure = sql_select('habillage','spip_eva_habillage','sauvegarde='.sql_quote($nom_sauvegarde));
		...
		$recherche_images_restaure=sql_select('type,nom_div,nom_image,pos_x,pos_y,repetition,attach','spip_eva_habillage_images','nom_habillage='.sql_quote($nom_sauvegarde));
		...
		$result_restaure=sql_select('*','spip_eva_habillage_themes','nom='.sql_quote($nom_sauvegarde));
		...
		$res['message_ok'] = 'La sauvegarde de l\'habillage <b>'.$nom_sauvegarde.'</b> a &eacute;t&eacute; restaur&eacute;e';
	...
	elseif ((_request('sauvegarder')=='Supprimer ?') AND (_request('supprimer_habillage')!='Defaut')) {
		...
		sql_delete('spip_eva_habillage_themes','nom='.sql_quote($nom_sauvegarde));
		sql_delete('spip_eva_habillage_images','nom_habillage='.sql_quote($nom_sauvegarde));
		$res['message_ok'] = 'La sauvegarde de l\'habillage <b>'.$nom_sauvegarde.'</b> a &eacute;t&eacute; supprim&eacute;e';
	}
	return $res;
}