...
function formulaires_evahabillage_pied_charger_dist(){
...
function formulaires_evahabillage_pied_traiter_dist(){
	...
	if (_request('submit_bloc_pied')) {
		$verif_post_bloc=sql_select('*','spip_eva_habillage_images',"type='bloc' AND nom_habillage='Defaut' AND attach='pied'");
	...
	if (_request('submit_bloc_perso_pied')) {
		sql_insertq('spip_eva_habillage_images',array(
			'type' => 'bloc',
			'nom_habillage' => 'Defaut',
			'nom_div' => _request('eva_mon_bloc_perso_pied'),
			'nom_image' => 'oui',
			'pos_x' => _request('eva_mon_bloc_perso_pos_x_pied'),
			'repetition' => 'perso',
			'attach' => 'pied'
		));
		$res['message_ok'] = 'Le squelette personnel <b>'._request('eva_mon_bloc_perso_pied').'</b> a &eacute;t&eacute; ins&eacute;r&eacute;';
	}
	//On traite enfin la suppression des noisettes perso préablement insérées
	if (_request('submit_supprime_skel_perso_pied')) {
		sql_delete('spip_eva_habillage_images',"id='"._request('eva_suppr_skel_perso_pied')."'");
