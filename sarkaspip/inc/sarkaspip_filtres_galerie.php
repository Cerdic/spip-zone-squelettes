...
//            - listing annuel, avec choix du mois de début de saison et du type d'affichage de la pagination
//            - planche contact
...
//               L'appel de ce filtre se fait à l'intérieur de la boucle ARTICLES de recensement des articles-albums.
//               La boucle doit etre triee en ordre chrono par via le critere {par date}
// Arguments :
//            - id : id de l'article à recenser comme album
//            - date : date de publication de l'article à recenser comme album
//            - titre : titre de l'article à recenser comme album
//            - logo : logo du premier document rencontré ou logo de l'article si aucun document
...
// =======================================================================================================================================
// Auteur: Smellup
// Fonction : Insertion d'une bande de pagination spécifique au type de galerie choisi
// Utilisation : Doit etre appelee apres le recensement à l'endroit ou aux endroits souhaitée de la page
// Arguments :
//            - page_choisie : si type_galerie=listing_annuel correspond a une annee sinon correspond au numero de planche
//            - filtre : -1 pour aucun filtre, 0 pour sans categorie, valeur>1 correspondant à l'id du mot-clé caracterisant la categorie
...
function galerie_listing_afficher($id_galerie, $annee_choisie, $filtre, $tri) {
	...
	$liste .= '<div class="listing_annuel">';
		...
		if ($albums[$j]['saison'] == $annee_choisie) {
			if (($filtre == '-1') || 
				...
				if ($nouveau_mois) {
					...
					$liste .= '<ul><li><a class="mois">'.$albums[$j]['nom_mois'].'&nbsp;'.$albums[$j]['annee'].'</a><ul>';
				}
				$mois_courant = $mois_album;
				$liste .= '<li><a class="article" href="spip.php?page=album&id_article='.$albums[$j]['id'].'">
...
function galerie_planche_afficher($id_galerie, $planche_choisie, $filtre, $tri) {
	...
	for ($i=1;$i<=$count_album;$i++) {
		...
		if ($albums[$j]['planche'] == $planche_choisie) {
			...
			$liste .= '<div class="album" style="min-height: 220px; _height: 220px;">';
