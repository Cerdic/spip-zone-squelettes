/**
 * jQEm v0.3 - Interact with the base 'em' unit.
 *   * http://davecardwell.co.uk/javascript/jquery/plugins/jquery-em/0.2/
 * 
 * Dave Cardwell <http://davecardwell.co.uk/>
 * Jacques PYRAT <http://www.pyrat.net/>
 *
...
new function() {
    ...
    var Private = {
        ...
        // if the plugin is currently polling for changes to the #jqem object,
     ...
     */
    function init() {
        // #jqem is an empty <div> element that we add to the document's
        ...
        Private.canExp = ( $('#jqem').style != undefined
                        && $('#jqem').style.setExpression != undefined );
    ...
    function trigger( force, args ) {
        ...
        if( force || $('#jqem').width() != Private.current ) {
            Private.previous = Private.current;
            Private.current  = $('#jqem').width();
    ...
    function start() {
        ...
        Private.current = Private.previous = $('#jqem').width();
        ...
        // polling the #jqem element for changes.
        if( Private.canExp ) {
            $('#jqem').style.setExpression( 'width', '$.jqem.update();' );
        } else {
    ...
    function stop() {
        ...
        if( Private.canExp ) {
            $('#jqem').style.removeExpression('width');
        } else {
