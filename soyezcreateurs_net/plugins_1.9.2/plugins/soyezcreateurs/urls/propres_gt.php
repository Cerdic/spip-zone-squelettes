...
function arborescence($id_rubrique) {
  if ($id_rubrique == 0) return '';
  $result = spip_query("SELECT id_rubrique, id_parent, url_propre FROM spip_rubriques WHERE id_rubrique=$id_rubrique");
  $ligne = spip_fetch_array($result);
  $id_parent = $ligne['id_parent'];
  $url = ($ligne['url_propre'] != '' ? $ligne['url_propre'] : _generer_url_propre('rubrique', $id_rubrique)) .'/';
  while ($id_parent) {
    $result = spip_query("SELECT id_rubrique, id_parent, url_propre FROM spip_rubriques WHERE id_rubrique=$id_parent");
    $ligne = spip_fetch_array($result);
    $id_parent = $ligne['id_parent'];
    $id_rubrique = $ligne['id_rubrique'];
    $url = ($ligne['url_propre'] ? $ligne['url_propre'] : _generer_url_propre('rubrique', $id_rubrique)) .'/'. $url;
  }
  return $url;
}
...
function generer_url_article($id_article) {
  ...
  if ($url) {
    $result = spip_query("SELECT id_rubrique FROM spip_articles WHERE id_article=$id_article");
    $ligne = spip_fetch_array($result);
    $id_rubrique = $ligne['id_rubrique'];
    return _debut_urls_propres . ($id_rubrique > 0 ? arborescence($id_rubrique) : '') . $url . _terminaison_urls_propres;
  } else
...
function generer_url_rubrique($id_rubrique) {
  ...
  if ($url) {
    ...
    $ligne = spip_fetch_array($result);
    $id_parent = $ligne['id_parent'];
    return _debut_urls_propres . ($id_parent > 0 ? arborescence($id_parent) : '') . $url .'/index'. _terminaison_urls_propres;
  } else
...
function generer_url_breve($id_breve) {
  ...
  if ($url) {
    $result = spip_query("SELECT id_rubrique FROM spip_breves WHERE id_breve=$id_breve");
    $ligne = spip_fetch_array($result);
    $id_rubrique = $ligne['id_rubrique'];
    return _debut_urls_propres . ($id_rubrique > 0 ? arborescence($id_rubrique) : '') . '+'. $url .'+'. _terminaison_urls_propres;
  } else
...
function generer_url_auteur($id_auteur) {
  ...
  if ($url)
    return _debut_urls_propres . 'auteurs/_'. $url .'_'. _terminaison_urls_propres;
  else
...
function generer_url_site($id_syndic) {
	...
	if ($url)
		return _debut_urls_propres . 'sites/@'.$url.'@'._terminaison_urls_propres;
	else
...
function recuperer_parametres_url(&$fond, $url) {
	...
	else if (preg_match(',^(.*)(/index)$,', $url_propre, $regs)) {
		$type = 'rubrique';
		$url_propre = $regs[1];
	}
	...
	else if (preg_match(',^auteurs/_(.*?)_?$,', $url_propre, $regs)) {
		$type = 'auteur';
		$url_propre = $regs[1];
	}
	else if (preg_match(',^sites/@(.*?)@?$,', $url_propre, $regs)) {
		$type = 'syndic';
		$url_propre = $regs[1];
	}
	else {
