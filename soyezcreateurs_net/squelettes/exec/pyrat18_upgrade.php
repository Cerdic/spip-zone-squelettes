...
function rename_groupe($groupe, $newgroupe) {
	...
	if ($id_groupe != 0) {
		// Mise à jour
		spip_query("UPDATE spip_groupes_mots SET titre='$newgroupe' WHERE id_groupe=$id_groupe");
	}
...
function config_site() {
	...
	$id_2Cols = id_mot('2Cols');
	$id_LayoutGala24 = id_mot('24. Two columns liquid, side fixed');
	if ($id_2Cols) {
		spip_query("UPDATE spip_mots_articles SET id_mot=$id_LayoutGala24 WHERE id_mot=$id_2Cols");
		spip_query("UPDATE spip_mots_rubriques SET id_mot=$id_LayoutGala24 WHERE id_mot=$id_2Cols");
		spip_query("DELETE FROM spip_mots WHERE id_mot=$id_2Cols");
	}

	rename_groupe("CategorieAgenda", "Thèmes de l\'Agenda");
}
