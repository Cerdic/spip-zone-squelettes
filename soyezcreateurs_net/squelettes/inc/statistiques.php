	...
	while ($row = spip_fetch_array($result)) {
		...
		if ($buff["host"]) {
			...
			if (strlen($buff["keywords"]) > 0) {
				...
				if (strlen($tmp) > 30)
					$tmp = "/".str_replace('&','&amp;',str_replace('&amp;','&',substr($tmp, 0, 27)))."...";
				else if (strlen($tmp) > 0)
					$tmp = str_replace('&','&amp;',str_replace('&amp;','&',"/$tmp"));
			}

			if ($tmp)
				$lesreferers[$numero][] = "<a href='".str_replace('&','&amp;',str_replace('&amp;','&',$referer))."'>$tmp</a>" . (($visites > 1)?" ($visites)":"");
			else
	...
	if (count($nbvisites) > 0) {
		...
		for (reset($nbvisites); $numero = key($nbvisites); next($nbvisites)) {
			...
			if ($lesdomaines[$numero] == "(email)") {
				...
				$aff .= $ret;
				$aff .= "<a href='http://".str_replace('&','&amp;',str_replace('&amp;','&',$lesurls[$numero]))."'><b><font color='$couleur_foncee'>".$lesdomaines[$numero]."</font></b></a>";
				...
				if (eregi("^(<a [^>]+>)([^ ]*)( \([0-9]+\))?", $lien, $regs)) {
					$lien = str_replace('&','&amp;',str_replace('&amp;','&',$regs[1])).$lesdomaines[$numero].$regs[2];
					if (!strpos($lien, '</a>')) $lien .= '</a>';
				} else
					$lien = "<a href='http://".$lesdomaines[$numero]."'>".$lesdomaines[$numero]."</a>";
				$aff .= "<b>".str_replace('&','&amp;',str_replace('&amp;','&',$lien))."</b>";
				$aff .= "</li>\n";
			}
		}
		$aff .= "</ul>";
