...
$modif = titres_articles($titre, $statut_article,$surtitre, $soustitre, $descriptif, $url_site, $nom_site, $flag_editable, $id_article, $id_rubrique);
 ...
 debut_cadre_couleur();
 echo formulaire_dater_m($id_article, $flag_editable, $statut_article, $date, $date_redac);
 fin_cadre_couleur();

 debut_cadre_couleur();
 echo formulaire_dater_even($id_article, $flag_editable, $statut_article, $date, $date_redac);
 fin_cadre_couleur();
...
if ($options == 'avancees' AND $GLOBALS['meta']["articles_mots"] != 'non') {
  echo formulaire_mots('options', $id_article, $cherche_mot, $select_groupe, $flag_editable);
}
...
// http://doc.spip.org/@formulaire_dater_m // modif pour le plugin scola SPIP
function formulaire_dater_m($id_article, $flag_editable, $statut_article, $date, $date_redac)
{
...
// http://doc.spip.org/@formulaire_dater_even plugin scolaSPIP
function formulaire_dater_even($id_article, $flag_editable, $statut_article, $date, $date_redac)
{
	global $spip_lang_left, $spip_lang_right, $options;
...
#			." " ._T('date_fmt_heures_minutes', array('h' =>$heure_redac, 'm'=>$minute_redac))
 ...
 "<b>" .
 "<span class='verdana1'>" .
 majuscules(_T('squelettesscola:texte_date_publication_anterieure')) .
'</span> '.
 $date_affichee .
 " " .
 aide('artdate_redac') .
 ...
 _T('squelettesscola:texte_date_publication_anterieure_nonaffichee').
 '</label>' .
...
// http://doc.spip.org/@formulaire_dater_m // modif pour le plugin scola SPIP
function formulaire_options_affichage($id_article, $flag_editable, $statut_article, $date, $date_redac)
{
	global $spip_lang_left, $spip_lang_right, $options;
  ...
  if ($statut_article <> 'publie') {
		...
		$js = "onchange=\"findObj_forcer('valider_options').style.visibility='visible';\"";
		$res = ajax_action_auteur("options", 
			"$id_article",
			'articles',
			"id_article=$id_article",
			(
 bouton_block_invisible("valider_options") .
 "<b><span class='verdana1'>".
 _T('squelettesscola:changer_options_article').
 '</span> ' . 
 
 "</b>".
 aide('aide_options') . 
 debut_block_invisible("valider_options") .
 "<div style='margin: 5px; margin-$spip_lang_left: 20px;'>" .
 afficher_jour($jour, "name='jour' size='1' class='fondl' $js", true) .
 afficher_mois($mois, "name='mois' size='1' class='fondl' $js", true) .
 afficher_annee($annee, "name='annee' size='1' class='fondl' $js") .
 ...
 afficher_heure($heure, "name='heure' size='1' class='fondl' $js") .
 afficher_minute($minute, "name='minute' size='1' class='fondl' $js") .
 "<span class='visible_au_chargement' id='valider_options'>" .
 " &nbsp;\n<input type='submit' class='fondo' value='".
 _T('bouton_changer')."' />" .
 "</span>" .
 "</div>" .
