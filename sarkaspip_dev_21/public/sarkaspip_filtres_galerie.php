		...
		if ($albums[$j]['saison'] == $annee_choisie) {
			if (($filtre == '-1') || 
				...
				if ($nouveau_mois) {
					...
					$liste .= '<ul><li><a class="mois-listing">'.$albums[$j]['nom_mois'].'&nbsp;'.$albums[$j]['annee'].'</a><ul>';
				}
				$mois_courant = $mois_album;
				$liste .= '<li><a class="article-listing" href="spip.php?page=album&id_article='.$albums[$j]['id'].'">
