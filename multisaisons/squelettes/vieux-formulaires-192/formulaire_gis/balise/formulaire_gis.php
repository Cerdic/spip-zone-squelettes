...
function balise_FORMULAIRE_GIS_dyn($id_rubrique) {
	...
	if($valider)
		...
		if ((isset($_FILES['commentimg'])) AND ($_FILES['commentimg']['error'] == "0")) {
    		...
    		move_uploaded_file($freshfile['tmp_name'], "/home/.cassimir/horaciogd/tracker.escoitar.org/plugins/gis/upload_form/".$freshfile['name']) OR die ("<p>Error!</p>");
			inc_ajouter_documents_dist ("/home/.cassimir/horaciogd/tracker.escoitar.org/plugins/gis/upload_form/".$freshfile['name'], $freshfile['name'], 'article', $id_novo_article , 'document', $id_document, $documents_actifs);
			unlink ("/home/.cassimir/horaciogd/tracker.escoitar.org/plugins/gis/upload_form/".$freshfile['name']);
		} else {
